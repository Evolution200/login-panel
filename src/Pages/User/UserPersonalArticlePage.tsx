...
const stateColorMap: Record<TaskState, string> = {
    ...
    [TaskState.Reject]: 'bg-red-100 text-red-800',
    [TaskState.Review]: 'bg-green-100 text-green-800',
    [TaskState.Accepted]: 'bg-purple-100 text-purple-800'
};

export function UserPersonalArticlePage() {
    const { username } = useUserStore();
    const { authorTasks, reviewerTasks, loading, error, fetchTasks } = useUserTaskStore();
    ...
    const renderTaskTable = (tasks: any[], title: string) => (
            ...
            <div className="space-y-8">
                <div className="bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 shadow-lg rounded-lg overflow-hidden">
                    <div className="px-4 py-5 sm:p-6">
                        <h2 className="text-3xl font-extrabold text-white">Personal Articles and Reviews</h2>
                        <p className="mt-1 text-xl text-white opacity-80">
                            View and manage your articles and review tasks
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-white shadow-md rounded-lg overflow-hidden">
                    <div className="px-4 py-5 sm:p-6">
                        ...
                        {!loading && !error && (
                            <>
                                {renderTaskTable(authorTasks, "My Articles")}
                                {renderTaskTable(reviewerTasks, "My Review Tasks")}
                            </>
                        )}
                    </div>
                </div>
