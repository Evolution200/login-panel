...
import { Application, fetchApplications } from './FetchApplication';
import { SendPostRequest } from './SendPost';

interface ApplicationManagementPageProps {
    Layout: React.ComponentType<{ currentPage: string; children: React.ReactNode }>;
    ReadTasksMessage: new () => any;
    FinishMessage: new (userName: string, allowed: boolean) => any;
    pageTitle: string;
    currentPage: string;
}
...
export function ApplicationManagementPage({
    ...
    const sendFinishMessage = async (userName: string, allowed: boolean) => {
        const message = new FinishMessage(userName, allowed);
        try {
            ...
            console.error('Failed to send FinishMessage:', error);
            throw error;
        }
    };
    ...
    const handleApprove = async (userName: string) => {
        try {
            await sendFinishMessage(userName, true);
            setErrorMessage('');
    ...
    const handleReject = async (userName: string) => {
        try {
            await sendFinishMessage(userName, false);
            setErrorMessage('');
            await loadApplications();
        } catch (error) {
